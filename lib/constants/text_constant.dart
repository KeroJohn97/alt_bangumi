// ignore_for_file: constant_identifier_names

mixin TextConstant {
  static const discover = 'Discover';
  static const timeCapsules = 'Time Capsules';
  static const favourite = 'Favourite';
  static const superUnfolded = 'Super Unfolded';
  static const timeMachine = 'Time Machine';
  static const ranking = 'Ranking';
  static const online = 'Online';
  static const anime = 'Anime';
  static const channel = 'Channel';
  static const entry = 'Entry';
  static const book = 'Book';
  static const music = 'Music';
  static const game = 'Game';
  static const film = 'Film';
  static const character = 'Character';
  static const user = 'User';
  static const accurate = 'Accurate';
  static const vague = 'Vague';
  static const search = 'Search';
  static const enquiry = 'Enquiry';
  static const viewInBrowser = 'View in Browser';
  static const clearHistory = 'Clear history';
  static const noHistory = 'No history';
  static const imageBroken = 'Image broken';
  static const enterKeywords = 'Enter keywords';
  static const nothingThere = 'It\'s like there\'s nothing there.';
  static const notAddedToFavourites = 'Not added to favourites';
  static const wished = 'Wished';
  static const watched = 'Watched';
  static const watching = 'Watching';
  static const onHold = 'On hold';
  static const dropped = 'Dropped';
  static const total = 'Total';
  static const chapter = 'Chapter';
  static const tag = 'Tag';
  static const summary = 'Summary';
  static const preview = 'Preview';
  static const more = 'More';
  static const details = 'Details';
  static const revise = 'Revise';
  static const rating = 'Rating';
  static const trend = 'Trend';
  static const perspective = 'Perspective';
  static const userRating = 'User rating';
  static const standardDeviation = 'Standard deviation';
  static const role = 'Role';
  static const productionStaff = 'Production Staff';
  static const associate = 'Associate';
  static const catalog = 'Catalog';
  static const log = 'Log';
  static const post = 'Post';
  static const dynamicString = 'Dynamic';
  static const comment = 'Comment';
  static const searchEntry = 'Search Entry';
  static const indexing = 'Indexing';
  static const today = 'Today';
  static const journal = 'Journal';
  static const ongoing = 'Ongoing';
  static const info = 'Info';
  static const stock = 'Stock';
  static const wiki = 'Wiki';
  static const almanac = 'Almanac';
  static const timeline = 'Timeline';
  static const netabare = 'netaba.re';
  static const localSMB = 'Local SMB';
  static const bilibiliSync = 'bilibili Sync';
  static const doubanSync = 'Douban Sync';
  static const backupCSV = 'Backup CSV';
  static const myCharacter = 'My Character';
  static const myCatalogue = 'My Catalogue';
  static const clipboard = 'Clipboard';
  static const loading = 'Loading';
  static const loadingWithDots = 'Loading ...';
  static const copyLink = 'Copy link';
  static const copyShare = 'Copy share';
  static const performance = 'Performance';
  static const whoHasCollectedIt = 'Who has collected it';
  static const recentlyParticipated = 'Recently participated';
  static const moreWorks = 'More works';
  static const producer = 'producer';
  static const mangaka = 'mangaka';
  static const artist = 'artist';
  static const seiyu = 'seiyu';
  static const writer = 'writer';
  static const illustrator = 'illustrator';
  static const actor = 'actor';
  static const pleaseGrantStoragePermission =
      'Please grant storage permission to the app to save';
  static const fileIsBeingSaved = 'File is being saved';
  static const subjectSharing = 'Subject Sharing';
  static const invalidURL = 'Uh-oh! It looks like the URL is invalid';
  static const unknown = 'Unknown';
  static const unknownError =
      'Oops! It seems like there might be a small hiccup';
  static const fileSaveTimeout = 'File save timeout';
  static const year = 'Year';
  static const month = 'Month';
  static const all = 'All';
  static const entire = 'Entire';
  static const type = 'Type';
  static const others = 'Others';
  static const tv = 'TV';
  static const web = 'WEB';
  static const ova = 'OVA';
  static const movie = 'Movie';
  static const comic = 'Comic';
  static const novel = 'Novel';
  static const illustration = 'Illustration';
  static const japaneseDrama = 'Japanese Drama';
  static const europeanNAmericanDramas = 'European & American Dramas';
  static const chineseDrama = 'Chinese Drama';
  static const pc = 'Personal Computer';
  static const ns = 'Nintendo Switch';
  static const ps5 = 'Play Station 5';
  static const ps4 = 'Play Station 4';
  static const psv = 'Play Station Vita';
  static const xboxSeries = 'Xbox Series X/S';
  static const xboxOne = 'Xbox One';
  static const wiiU = 'Wii U';
  static const ps3 = 'Play Station 3';
  static const xbox360 = 'Xbox 360';
  static const threeDS = 'Nintendo 3D Dual Screen';
  static const psp = 'Play Station Portable';
  static const wii = 'Wii';
  static const nds = 'Nintendo Dual Screen';
  static const ps2 = 'Play Station 2';
  static const xbox = 'Xbox';
  static const mac = 'Macbook';
  static const ps = 'Play Station 1';
  static const gba = 'Game Boy Advanced';
  static const gb = 'Game Boy';
  static const fc = 'Famicom';
  static const numberOfAnnotations = 'Number of Annotations';
  static const name = 'Name';
  static const date = 'Date';
  static const upcoming = 'Upcoming';
  static const premiere = 'Premiere';
  static const duration = 'Duration';
  static const trackList = 'TrackList';
  static const disc = 'Disc';
  static const sort = 'Sort';
  static const translation = 'Translation';
  static const chooseALanguage = 'Choose a language';
  static const settings = 'Settings';
  static const clearCache = 'Clear Cache';
  static const searchHistory = 'Search History';
  static const searchResultHistory = 'Search Result History';
  static const defaultSearchSubjectOption = 'Default Search Subject Option';
  static const homeAnimeList = 'Home Anime List';
  static const translationHistory = 'Translation History';
  static const subjectDetailList = 'Subject Detail List';
  static const areYouSureContinue = 'Are you sure you would like to continue?';
  static const no = 'No';
  static const yes = 'Yes';
  static const applicationLanguage = 'Application Language';
  static const english = 'English';
  static const simplifiedChinese = 'Simplified Chinese';
  static const traditionalChinese = 'Traditional Chinese';
  static const noInternetConnection = 'No Internet connection';
  static const copied = 'Copied';
  static const withoutAName = '(Name does not exist)';
  static const withoutADate = '(Date does not exist)';
  static const inputLessThanRatings = '(Less than %a ratings)';
  static const inputPeopleRatings = '(%a ratings)';
  static const inputDateString = '%a-%a-%a';
  static const theFileSaved = 'The file saved in directory (%a)';
  static const charactersBelong = '%a\'s Characters';
  static const productionStaffsBelong = '%a\'s Production Staffs';
  static const somethingCleared = '%a Cleared';
  static const somethingDoesNotExist = '%a Does not exist';
  static const somethingUpdatedAt = 'Updated at %a';

  static const EN = {
    discover: 'Discover',
    timeCapsules: 'Time Capsules',
    favourite: 'Favourite',
    superUnfolded: 'Super Unfolded',
    timeMachine: 'Time Machine',
    ranking: 'Ranking',
    online: 'Online',
    anime: 'Anime',
    channel: 'Channel',
    entry: 'Entry',
    book: 'Book',
    music: 'Music',
    game: 'Game',
    film: film,
    character: 'Character',
    user: 'User',
    accurate: 'Accurate',
    vague: 'Vague',
    search: 'Search',
    enquiry: 'Enquiry',
    viewInBrowser: 'View in Browser',
    clearHistory: 'Clear history',
    noHistory: noHistory,
    imageBroken: imageBroken,
    enterKeywords: enterKeywords,
    nothingThere: nothingThere,
    notAddedToFavourites: notAddedToFavourites,
    wished: wished,
    watched: watched,
    watching: watching,
    onHold: onHold,
    dropped: dropped,
    total: total,
    chapter: chapter,
    tag: tag,
    summary: summary,
    preview: preview,
    more: more,
    details: details,
    revise: revise,
    rating: rating,
    trend: trend,
    perspective: perspective,
    userRating: userRating,
    standardDeviation: standardDeviation,
    role: role,
    productionStaff: productionStaff,
    associate: associate,
    catalog: catalog,
    log: log,
    post: post,
    dynamicString: dynamic,
    comment: comment,
    searchEntry: searchEntry,
    indexing: indexing,
    today: today,
    journal: journal,
    ongoing: ongoing,
    info: info,
    stock: stock,
    wiki: wiki,
    almanac: almanac,
    timeline: timeline,
    netabare: netabare,
    localSMB: localSMB,
    bilibiliSync: bilibiliSync,
    doubanSync: doubanSync,
    backupCSV: backupCSV,
    myCharacter: myCharacter,
    myCatalogue: myCatalogue,
    clipboard: clipboard,
    loading: loading,
    loadingWithDots: loadingWithDots,
    copyLink: copyLink,
    copyShare: copyShare,
    performance: performance,
    whoHasCollectedIt: whoHasCollectedIt,
    recentlyParticipated: recentlyParticipated,
    moreWorks: moreWorks,
    producer: producer,
    mangaka: mangaka,
    artist: artist,
    seiyu: seiyu,
    writer: writer,
    illustrator: illustrator,
    actor: actor,
    pleaseGrantStoragePermission: pleaseGrantStoragePermission,
    fileIsBeingSaved: fileIsBeingSaved,
    subjectSharing: subjectSharing,
    invalidURL: invalidURL,
    unknownError: unknownError,
    fileSaveTimeout: fileSaveTimeout,
    year: year,
    month: month,
    all: all,
    entire: entire,
    type: type,
    others: others,
    tv: tv,
    web: web,
    ova: ova,
    movie: movie,
    comic: comic,
    novel: novel,
    illustration: illustration,
    japaneseDrama: japaneseDrama,
    europeanNAmericanDramas: europeanNAmericanDramas,
    chineseDrama: chineseDrama,
    pc: pc,
    ns: ns,
    ps5: ps5,
    ps4: ps4,
    psv: psv,
    xboxSeries: xboxSeries,
    xboxOne: xboxOne,
    wiiU: wiiU,
    ps3: ps3,
    xbox360: xbox360,
    threeDS: threeDS,
    psp: psp,
    wii: wii,
    nds: nds,
    ps2: ps2,
    xbox: xbox,
    mac: mac,
    ps: ps,
    gba: gba,
    gb: gb,
    fc: fc,
    numberOfAnnotations: numberOfAnnotations,
    name: name,
    date: date,
    upcoming: upcoming,
    premiere: premiere,
    duration: duration,
    trackList: trackList,
    disc: disc,
    sort: sort,
    translation: translation,
    chooseALanguage: chooseALanguage,
    settings: settings,
    clearCache: clearCache,
    searchHistory: searchHistory,
    searchResultHistory: searchResultHistory,
    defaultSearchSubjectOption: defaultSearchSubjectOption,
    homeAnimeList: homeAnimeList,
    translationHistory: translationHistory,
    subjectDetailList: subjectDetailList,
    areYouSureContinue: areYouSureContinue,
    no: no,
    yes: yes,
    applicationLanguage: applicationLanguage,
    english: english,
    simplifiedChinese: simplifiedChinese,
    traditionalChinese: traditionalChinese,
    noInternetConnection: noInternetConnection,
    copied: copied,
    withoutAName: '(Name does not exist)',
    withoutADate: '(Date does not exist)',
    inputLessThanRatings: inputLessThanRatings,
    inputPeopleRatings: inputPeopleRatings,
    inputDateString: inputDateString,
    theFileSaved: theFileSaved,
    charactersBelong: charactersBelong,
    productionStaffsBelong: productionStaffsBelong,
    somethingCleared: somethingCleared,
    somethingDoesNotExist: somethingDoesNotExist,
    somethingUpdatedAt: somethingUpdatedAt,
  };

  static const ZH_CN = {
    discover: '发现',
    timeCapsules: '时间胶囊',
    favourite: '收藏',
    superUnfolded: '超展开',
    timeMachine: '时光机',
    ranking: '排行榜',
    online: '在线',
    anime: '动画',
    channel: '频道',
    entry: '条目',
    book: '书籍',
    music: '音乐',
    game: '游戏',
    film: '影视',
    character: '人物',
    user: '用户',
    accurate: '精确',
    vague: '模糊',
    search: '搜索',
    enquiry: '查询',
    viewInBrowser: '浏览器查看',
    clearHistory: '清除历史',
    noHistory: '无历史记录',
    imageBroken: '图像损坏',
    enterKeywords: '输入关键字',
    nothingThere: '好像什么都没有',
    notAddedToFavourites: '未加入收藏',
    wished: '想看',
    watched: '看过',
    watching: '在看',
    onHold: '搁置',
    dropped: '抛弃',
    total: '总',
    chapter: '章节',
    tag: '标签',
    summary: '简介',
    preview: '预览',
    more: '更多',
    details: '详情',
    revise: '修订',
    rating: '评分',
    trend: '趋势',
    perspective: '透视',
    userRating: '用户评分',
    standardDeviation: '标准差',
    role: '角色',
    productionStaff: '制作人员',
    associate: '关联',
    catalog: '目录',
    log: '日志',
    post: '帖子',
    dynamicString: '动态',
    comment: '评论',
    searchEntry: '找条目',
    indexing: '索引',
    today: '每日放送',
    journal: '日志',
    ongoing: '新番',
    info: '资讯',
    stock: '小圣杯',
    wiki: '维基人',
    almanac: '年鉴',
    timeline: '时间线',
    netabare: 'netaba.re',
    localSMB: '本地管理',
    bilibiliSync: 'bilibili 同步',
    doubanSync: '豆瓣同步',
    backupCSV: '本地备份',
    myCharacter: '我的人物',
    myCatalogue: '我的目录',
    clipboard: '剪贴板',
    loading: '载入中',
    loadingWithDots: '载入中 。。。',
    copyLink: '复制链接',
    copyShare: '复制分享',
    performance: '出演',
    whoHasCollectedIt: '谁收藏了',
    recentlyParticipated: '最近参与',
    moreWorks: '更多作品',
    producer: '制作人',
    mangaka: '漫画家',
    artist: '艺术家',
    seiyu: '声优',
    writer: '作家',
    illustrator: '插画家',
    actor: '演员',
    pleaseGrantStoragePermission: '请授予应用存储权限以保存',
    fileIsBeingSaved: '文件保存中',
    subjectSharing: '主题分享',
    invalidURL: "哎呀！看起来URL无效",
    unknownError: "糟糕！似乎出了点小问题",
    fileSaveTimeout: '文件保存超时',
    year: '年',
    month: '月',
    all: '全部',
    entire: '全部',
    type: '类型',
    others: "其他",
    tv: "TV",
    web: "WEB",
    ova: "OVA",
    movie: "剧场版",
    comic: "漫画",
    novel: "小说",
    illustration: "画集",
    japaneseDrama: "日剧",
    europeanNAmericanDramas: "欧美剧",
    chineseDrama: "中剧",
    pc: "PC",
    ns: "NS",
    ps5: "PS5",
    ps4: "PS4",
    psv: "PSV",
    xboxSeries: "Xbox Series X/S",
    xboxOne: "Xbox One",
    wiiU: "Wii U",
    ps3: "PS3",
    xbox360: "Xbox 360",
    threeDS: "3DS",
    psp: "PSP",
    wii: "Wii",
    nds: "NDS",
    ps2: "PS2",
    xbox: "Xbox",
    mac: "Mac",
    ps: "PS",
    gba: "GBA",
    gb: "GB",
    fc: "FC",
    numberOfAnnotations: '标注数',
    name: '名称',
    date: '日期',
    upcoming: '即将推出',
    premiere: '首播',
    duration: '时长',
    trackList: '曲目列表',
    disc: '光盘',
    sort: '排序',
    translation: '翻译',
    chooseALanguage: '选择语言',
    settings: '设置',
    clearCache: '清除缓存',
    searchHistory: '搜索历史记录',
    searchResultHistory: '搜索结果历史记录',
    defaultSearchSubjectOption: '默认搜索主题选项',
    homeAnimeList: '主页动漫列表',
    translationHistory: '翻译历史记录',
    subjectDetailList: '主题列表',
    areYouSureContinue: '你确定要继续吗？',
    no: '不是',
    yes: '是',
    applicationLanguage: '应用语言',
    english: '英文',
    simplifiedChinese: '简体中文',
    traditionalChinese: '繁体中文',
    noInternetConnection: '没有网络连接',
    copied: '已复制',
    withoutAName: '(名字不存在)',
    withoutADate: '(日期不存在)',
    inputLessThanRatings: '(少于%a人评分)',
    inputPeopleRatings: '(%a人评分)',
    inputDateString: '%a年%a月%a日',
    theFileSaved: '文件已保存 (%a)',
    charactersBelong: '%a的角色',
    productionStaffsBelong: '%a的制作人员',
    somethingCleared: '%a已清除',
    somethingDoesNotExist: '%a不存在',
    somethingUpdatedAt: '更新于%a',
  };

  static const ZH_TW = {
    discover: '發現',
    timeCapsules: '時間膠囊',
    favourite: '收藏',
    superUnfolded: '超展開',
    timeMachine: '時光機',
    ranking: '排行榜',
    online: '在綫',
    anime: '動畫',
    channel: '頻道',
    entry: '條目',
    book: '書籍',
    music: '音樂',
    game: '游戲',
    film: '影視',
    character: '人物',
    user: '用戶',
    accurate: '精確',
    vague: '模糊',
    search: '搜索',
    enquiry: '查詢',
    viewInBrowser: '瀏覽器查看',
    clearHistory: '清除歷史',
    noHistory: '無歷史記錄',
    imageBroken: '圖像損壞',
    enterKeywords: '輸入關鍵字',
    nothingThere: '好像什麼都沒有',
    notAddedToFavourites: '未加入收藏',
    wished: '想看',
    watched: '看過',
    watching: '在看',
    onHold: '擱置',
    dropped: '拋棄',
    total: '總',
    chapter: '章節',
    tag: '標籤',
    summary: '簡介',
    preview: '預覽',
    more: '更多',
    details: '詳情',
    revise: '修訂',
    rating: '評分',
    trend: '趨勢',
    perspective: '透視',
    userRating: '用戶評分',
    standardDeviation: '標準差',
    role: '角色',
    productionStaff: '製作人員',
    associate: '關聯',
    catalog: '目錄',
    log: '日誌',
    post: '帖子',
    dynamicString: '動態',
    comment: '評論',
    searchEntry: '找條目',
    indexing: '索引',
    today: '每日放送',
    journal: '日誌',
    ongoing: '新番',
    info: '資訊',
    stock: '小聖杯',
    wiki: '維基人',
    almanac: '年鑒',
    timeline: '時間線',
    netabare: 'netaba.re',
    localSMB: '本地管理',
    bilibiliSync: 'bilibili 同步',
    doubanSync: '豆瓣同步',
    backupCSV: '本地備份',
    myCharacter: '我的人物',
    myCatalogue: '我的目錄',
    clipboard: '剪貼板',
    loading: '載入中',
    loadingWithDots: '載入中 。。。',
    copyLink: '複製鏈接',
    copyShare: '複製分享',
    performance: '出演',
    whoHasCollectedIt: '誰收藏了',
    recentlyParticipated: '最近參與',
    moreWorks: '更多作品',
    producer: '製作人',
    mangaka: '漫畫家',
    artist: '藝術家',
    seiyu: '聲優',
    writer: '作家',
    illustrator: '插畫家',
    actor: '演員',
    pleaseGrantStoragePermission: '請授予應用存儲權限以保存',
    fileIsBeingSaved: '文件保存中',
    subjectSharing: '主題分享',
    invalidURL: '哎呀！看起來URL無效',
    unknownError: '糟糕！似乎出了點小問題',
    fileSaveTimeout: '文件保存超時',
    year: '年',
    month: '月',
    all: '全部',
    entire: '全部',
    type: '類型',
    pc: 'PC',
    ns: 'NS',
    ps5: 'PS5',
    ps4: 'PS4',
    psv: 'PSV',
    xboxSeries: 'Xbox Series X/S',
    xboxOne: 'Xbox One',
    wiiU: 'Wii U',
    ps3: 'PS3',
    xbox360: 'Xbox 360',
    threeDS: '3DS',
    psp: 'PSP',
    wii: 'Wii',
    nds: 'NDS',
    ps2: 'PS2',
    xbox: 'Xbox',
    mac: 'Mac',
    ps: 'PS',
    gba: 'GBA',
    gb: 'GB',
    fc: 'FC',
    numberOfAnnotations: '標註數',
    name: '名稱',
    date: '日期',
    trackList: '曲目列表',
    disc: '光盤',
    sort: '排序',
    translation: '翻譯',
    chooseALanguage: '選擇語言',
    settings: '設置',
    clearCache: '清除緩存',
    searchHistory: '搜索歷史記錄',
    searchResultHistory: '搜索結果歷史記錄',
    defaultSearchSubjectOption: '默認搜索主題選項',
    homeAnimeList: '主頁動漫列表',
    translationHistory: '翻譯歷史記錄',
    subjectDetailList: '主題列表',
    areYouSureContinue: '你確定要繼續嗎？',
    no: '不是',
    yes: '是',
    applicationLanguage: '應用語言',
    english: '英文',
    simplifiedChinese: '簡體中文',
    traditionalChinese: '繁體中文',
    noInternetConnection: '沒有網路連接',
    copied: '已複製',
/////////////////////////////////////////////////////////
    withoutAName: '(名字不存在)',
    withoutADate: '(日期不存在)',
    inputLessThanRatings: '(少於%a人評分)',
    inputDateString: '%a年%a月%a日',
    theFileSaved: '文件已保存 (%a)',
    charactersBelong: '%a的角色',
    productionStaffsBelong: '%a的製作人員',
    somethingCleared: '%a已清除',
    somethingDoesNotExist: '%a不存在',
    somethingUpdatedAt: '更新於%a',
    /////////////////////////////////////////////////////////
  };

  static const JA = {};
}
